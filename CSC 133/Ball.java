...
class Ball {
    ...
    private float mBallSize;                            // Combines width and height into a single variable
    ...
    Ball(int screenX){
        ...
        mBallSize = screenX / 100;                       // comboned the width and height to make it more omptizmized
    ...
    void update(long fps){
        ...
        mRect.right = mRect.left + mBallSize;         // changed to ballSize
        mRect.bottom = mRect.top + mBallSize;         // changed to ballSize
    }
    ...
    void reset(int x, int y){
        ...
        mRect.left = x / 2;
        mRect.top = 0;
        mRect.right = x / 2 + mBallSize;          // changed to ballSize
        mRect.bottom = mBallSize;                 // changed to ballSize
    ...
    void increaseVelocity(){
        ...
        mXVelocity *= 1.1f;                     // changed this for optimizations
        mYVelocity *= 1.1f;                     // same for this 
    }
    ...
    void batBounce(RectF batPosition){
        ...
        float batCenter = batPosition.left + (batPosition.width() / 2);  // code changed to go with just size instead of width & height
        float ballCenter = mRect.left + (mBallSize / 2);                 // same thing done here
