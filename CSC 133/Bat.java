...
class Bat {
    ...
    void update(long fps){
        ...
        float moveAmount = mBatSpeed / fps;                               // Calculate the movement amount based on the bat speed and frame rate

        
        if (mBatMoving == LEFT) {
            mXCoord = Math.max(0, mXCoord - moveAmount);                  // Removed and combined IF Statement
        } else if (mBatMoving == RIGHT) {
            mXCoord = Math.min(mScreenX - mLength, mXCoord + moveAmount); // MIN and MAX to make sure bat stays on screen
        }
